...
impl LiveHook for App {
    fn after_new_from_doc(&mut self, _cx: &mut Cx) {
        ...
        self.app_registry.register(MoFaPrimeSpeechApp::info());
...
impl App {
        ...
        self.ui
            .view(ids!(
                body.dashboard_wrapper
                    ...
                    .main_content
                    .content
                    .tts_primespeech_page
            ))
            .apply_over(
                cx,
                live! { visible: (current == Some(PageId::TTSPrimeSpeech)) },
            );
        self.ui
            .view(ids!(
                body.dashboard_wrapper
    ...
    fn update_hero_title(&mut self, cx: &mut Cx, page: PageId) {
        let (title, description) = match page {
            ...
            PageId::TTS => ("TTS", "Text to Speech conversion with Kokoro"),
