...
impl TTSPlayer {
    ...
    pub fn new() -> Self {
        let sample_rate = 24000; // Default sample rate for kokoro/primespeech, dynamic adaptation handled in thread
...
fn run_audio_thread(
    ...
    let stream_result = match device.default_output_config().unwrap().sample_format() {
        cpal::SampleFormat::F32 => build_stream_for_format::<f32>(
            &device, &config, buffer_clone, is_playing_clone, Arc::clone(&state), output_channels, sample_pos, playback_rate
        ),
