...
//! - Config: ~/.dora/friend/custom_voices.json
//! - Audio: ~/.dora/friend/custom_voices/{voice_id}/ref.wav
...
/// Get the base directory for Friend data
pub fn get_friend_dir() -> PathBuf {
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    home.join(".dora").join("friend")
}

/// Get the custom voices config file path
pub fn get_config_path() -> PathBuf {
    get_friend_dir().join("custom_voices.json")
}

/// Get the custom voices audio directory
pub fn get_custom_voices_dir() -> PathBuf {
    get_friend_dir().join("custom_voices")
}
...
pub fn ensure_directories() -> std::io::Result<()> {
    let friend_dir = get_friend_dir();
    if !friend_dir.exists() {
        fs::create_dir_all(&friend_dir)?;
    }
...
pub fn get_reference_audio_path(voice: &Voice) -> Option<PathBuf> {
    ...
    voice
        ...
        .as_ref()
        .map(|rel_path| get_custom_voices_dir().join(rel_path))
}
